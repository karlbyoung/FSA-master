...
CREATE OR REPLACE FUNCTION DEV.${FSA_CURRENT_SCHEMA}.LATEST_DEMAND_PREV_ASSIGNED (MAX_DATE timestamp_ltz)
  RETURNS TABLE (
...
AS $$
    ...
    from DEV.${FSA_CURRENT_SCHEMA}.demand_prev_assigned_historical t1
    join (
      select distinct
        pk_id,
        max(last_modified) over (partition by pk_id) max_mod_date,
        max(fsa_insert_date) over (partition by pk_id) max_fsa_date
          from DEV.${FSA_CURRENT_SCHEMA}.demand_prev_assigned_historical
...
CREATE OR REPLACE FUNCTION DEV.${FSA_CURRENT_SCHEMA}.LATEST_DEMAND_PREV_ASSIGNED ()
  RETURNS TABLE (
	PK_ID VARCHAR(16777216),
	UNIQUE_KEY FLOAT,
	COMPONENT_ITEM_ID FLOAT,
...
AS $$
	select * from TABLE(DEV.${FSA_CURRENT_SCHEMA}.LATEST_DEMAND_PREV_ASSIGNED(current_timestamp()))
$$;

CREATE OR REPLACE PROCEDURE DEV.${FSA_CURRENT_SCHEMA}.MAKE_DEMAND_PREV_ASSIGNED(MAX_DATE timestamp_ltz,TARGET_TABLE text)
  RETURNS TABLE()
  LANGUAGE SQL
  EXECUTE AS CALLER
AS $$
  BEGIN
  LET rs RESULTSET := (CREATE OR REPLACE TABLE IDENTIFIER(:TARGET_TABLE) AS SELECT * FROM TABLE(DEV.${FSA_CURRENT_SCHEMA}.LATEST_DEMAND_PREV_ASSIGNED(:MAX_DATE)));
  return TABLE(rs);
  END;
$$;

CREATE OR REPLACE PROCEDURE DEV.${FSA_CURRENT_SCHEMA}.MAKE_DEMAND_PREV_ASSIGNED(MAX_DATE timestamp_ltz)
  RETURNS TABLE()
  LANGUAGE SQL
  EXECUTE AS CALLER
AS $$
  BEGIN
  LET rs RESULTSET := (CALL DEV.${FSA_CURRENT_SCHEMA}.MAKE_DEMAND_PREV_ASSIGNED(:MAX_DATE,'DEV.${FSA_CURRENT_SCHEMA}.DEMAND_PREV_ASSIGNED'));
  return TABLE(rs);
  END;
$$;

CREATE OR REPLACE PROCEDURE DEV.${FSA_CURRENT_SCHEMA}.MAKE_DEMAND_PREV_ASSIGNED()
  RETURNS TABLE()
  LANGUAGE SQL
  EXECUTE AS CALLER
AS $$
  BEGIN
  LET rs RESULTSET := (CALL DEV.${FSA_CURRENT_SCHEMA}.MAKE_DEMAND_PREV_ASSIGNED(current_timestamp()));
  return TABLE(rs);
  END;
$$;
