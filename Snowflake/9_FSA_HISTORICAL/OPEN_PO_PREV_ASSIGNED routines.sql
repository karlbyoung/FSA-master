...
CREATE OR REPLACE FUNCTION DEV.${vj_fsa_schema}.LATEST_OPEN_PO_PREV_ASSIGNED (MAX_DATE timestamp_ltz)
  RETURNS TABLE (
...
AS $$
    ...
    from DEV.${vj_fsa_schema}.open_po_prev_assigned_historical t1
    join (
      select distinct
        pk_id,
        max(last_modified) over (partition by pk_id) max_mod_date,
        max(fsa_insert_date) over (partition by pk_id) max_fsa_date
          from DEV.${vj_fsa_schema}.open_po_prev_assigned_historical
...
CREATE OR REPLACE FUNCTION DEV.${vj_fsa_schema}.LATEST_OPEN_PO_PREV_ASSIGNED ()
  RETURNS TABLE (
	PK_ID VARCHAR(16777216),
	HASH_VALUE NUMBER(38,0),
	INSERT_DATE TIMESTAMP_LTZ(9),
...
AS $$
	select * from TABLE(DEV.${vj_fsa_schema}.LATEST_OPEN_PO_PREV_ASSIGNED(current_timestamp()))
$$;

CREATE OR REPLACE FUNCTION DEV.${vj_fsa_schema}.LATEST_OPEN_PO_PREV_ASSIGNED (time_as_text text)
  RETURNS TABLE (
	PK_ID VARCHAR(16777216),
	HASH_VALUE NUMBER(38,0),
	INSERT_DATE TIMESTAMP_LTZ(9),
...
AS $$
	select * from TABLE(DEV.${vj_fsa_schema}.LATEST_OPEN_PO_PREV_ASSIGNED(try_to_timestamp_ltz(time_as_text)))
$$;

CREATE OR REPLACE PROCEDURE DEV.${vj_fsa_schema}.MAKE_OPEN_PO_PREV_ASSIGNED(MAX_DATE timestamp_ltz,TARGET_TABLE text)
  RETURNS TABLE()
  LANGUAGE SQL
  EXECUTE AS CALLER
AS $$
  BEGIN
  LET rs RESULTSET := (CREATE OR REPLACE TABLE IDENTIFIER(:TARGET_TABLE) AS SELECT * FROM TABLE(DEV.${vj_fsa_schema}.LATEST_OPEN_PO_PREV_ASSIGNED(:MAX_DATE)));
  return TABLE(rs);
  END;
$$;

CREATE OR REPLACE PROCEDURE DEV.${vj_fsa_schema}.MAKE_OPEN_PO_PREV_ASSIGNED(MAX_DATE timestamp_ltz)
  RETURNS TABLE()
  LANGUAGE SQL
  EXECUTE AS CALLER
AS $$
  BEGIN
  LET rs RESULTSET := (CALL DEV.${vj_fsa_schema}.MAKE_OPEN_PO_PREV_ASSIGNED(:MAX_DATE,'DEV.${vj_fsa_schema}.OPEN_PO_PREV_ASSIGNED'));
  return TABLE(rs);
  END;
$$;

CREATE OR REPLACE PROCEDURE DEV.${vj_fsa_schema}.MAKE_OPEN_PO_PREV_ASSIGNED()
  RETURNS TABLE()
  LANGUAGE SQL
  EXECUTE AS CALLER
AS $$
  BEGIN
  LET rs RESULTSET := (CALL DEV.${vj_fsa_schema}.MAKE_OPEN_PO_PREV_ASSIGNED(current_timestamp()));
  return TABLE(rs);
  END;
$$;

CREATE OR REPLACE FUNCTION DEV.${vj_fsa_schema}.DATES_OPEN_PO_PREV_ASSIGNED()
  RETURNS TABLE(
...
CREATE OR REPLACE FUNCTION DEV.${vj_fsa_schema}.LATEST_OPEN_PO_PREV_ASSIGNED (recent_or_inorder number)
  RETURNS TABLE (
	PK_ID VARCHAR(16777216),
	HASH_VALUE NUMBER(38,0),
	INSERT_DATE TIMESTAMP_LTZ(9),
...
AS $$
    ...
    from DEV.${vj_fsa_schema}.OPEN_PO_PREV_ASSIGNED_HISTORICAL p
    join table(DEV.${vj_fsa_schema}.DATES_OPEN_PO_PREV_ASSIGNED()) d
